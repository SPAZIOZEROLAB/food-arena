...
import {publishReceipt} from '../../fuji';
...
const holdSql=`SELECT COALESCE(SUM(o.quantity),0) FROM offers o JOIN requests r ON r.id=o.request_id WHERE o.menu_id=m.id AND ((o.status='offered' AND o.expires>MAX(?,${DBNOW}) AND r.status='open' AND r.pickup_by>MAX(?,${DBNOW})) OR (o.status='accepted' AND r.chosen=o.id AND r.status='accepted' AND r.pickup_by>MAX(?,${DBNOW})))`;
...
export async function GET(req:Request){try{
 ...
 if(view==='merchant'){const {u,v}=await ownedVenue();return respond({user:u.displayName,venue:v,menu:v?await menuFor(v.id,n):[],requests:v?(await db().prepare(`SELECT ${publicRequest} FROM requests WHERE zone=? AND demo=? AND status='open' AND pickup_by>? ORDER BY created DESC LIMIT 80`).bind(v.zone,v.demo,n).all()).results:[],offers:v?(await db().prepare(`SELECT o.*,r.wish,r.budget,r.pickup_by,r.status AS request_status FROM offers o JOIN requests r ON r.id=o.request_id WHERE o.venue_id=? AND o.created>? ORDER BY o.created DESC LIMIT 100`).bind(v.id,n-86400000).all<any>()).results.map(({code,...safe})=>safe):[]})}
 if(view==='request'){
  ...
  for(const o of offers)if(!mine||r.chosen!==o.id)delete o.code;
  return respond({request:r,mine,offers,now:n});
 }
...
export async function POST(req:Request){try{
 ...
 if(b.action==='request'){
  ...
  const active=await db().prepare("SELECT COUNT(*) AS n FROM requests WHERE owner_hash=? AND status IN ('open','accepted') AND pickup_by>?").bind(owner,n).first<any>();if(active.n>=3)fail('Hai già tre richieste aperte. Chiudi una richiesta prima di crearne un’altra.',429);
 ...
 if(b.action==='offer'){
  ...
  const inserted=await db().prepare(`INSERT OR IGNORE INTO offers(id,request_id,venue_id,menu_id,title,description,price,quantity,ready_at,expires,created,status,code) SELECT ?,?,?,?,?,?,?,?,?,?,?,?,? FROM menu m WHERE m.id=? AND m.active=1 AND m.stock-(${holdSql})>=? AND EXISTS(SELECT 1 FROM requests WHERE id=? AND status='open' AND pickup_by>MAX(?,${DBNOW})) AND ?>${DBNOW}`).bind(id,r.id,v.id,m.id,m.name,m.description,total,r.quantity,ready,expires,n,'offered',code,m.id,n,n,n,r.quantity,r.id,n,expires).run();if(!inserted.meta.changes)fail('Hai già risposto oppure la quantità è appena stata riservata.');return respond({id});
 }
 if(b.action==='accept'||b.action==='cancel'){
  ...
  const result=await db().batch([db().prepare(`UPDATE requests SET status='accepted',chosen=? WHERE id=? AND status='open' AND pickup_by>MAX(?,${DBNOW}) AND EXISTS(SELECT 1 FROM offers WHERE id=? AND status='offered' AND expires>MAX(?,${DBNOW}))`).bind(o.id,r.id,n,o.id,n),db().prepare("UPDATE offers SET status='accepted' WHERE id=? AND EXISTS(SELECT 1 FROM requests WHERE id=? AND chosen=? AND status='accepted')").bind(o.id,r.id,o.id),db().prepare("UPDATE offers SET status='cancelled' WHERE request_id=? AND id<>? AND status='offered' AND EXISTS(SELECT 1 FROM requests WHERE id=? AND chosen=? AND status='accepted')").bind(r.id,o.id,r.id,o.id)]);if(!result[0].meta.changes)fail('Un’altra scelta è già stata registrata.');return respond({ok:true});
 }
 ...
 if(b.action==='collect'||b.action==='demoCollect'){
  ...
  const proof=await hash(JSON.stringify({offer:o.id,price:o.price,created:o.created,nonce:o.code}));
  const result=await db().batch([db().prepare(`UPDATE menu SET stock=stock-? WHERE id=? AND stock>=? AND EXISTS(SELECT 1 FROM offers o JOIN requests r ON r.id=o.request_id WHERE o.id=? AND o.status='accepted' AND r.status='accepted' AND r.pickup_by>${DBNOW})`).bind(o.quantity,o.menu_id,o.quantity,o.id),db().prepare("UPDATE offers SET status='collected',proof_hash=? WHERE id=? AND status='accepted' AND changes()>0").bind(proof,o.id),db().prepare("UPDATE requests SET status='collected' WHERE id=? AND chosen=? AND EXISTS(SELECT 1 FROM offers WHERE id=? AND status='collected')").bind(o.request_id,o.id,o.id)]);if(!result[1].meta.changes)fail('Ritiro già registrato o quantità non valida.');return respond({ok:true,proofHash:proof});
 }
 if(b.action==='attest'||b.action==='demoAttest'){
  let v:any;if(b.action==='demoAttest'){const t=cookie(req);if(!t)fail('Usa il dispositivo della prova.',403);v=await db().prepare("SELECT v.* FROM venues v JOIN offers o ON o.venue_id=v.id JOIN requests r ON r.id=o.request_id WHERE o.id=? AND r.owner_hash=? AND r.demo=1 AND v.owner LIKE 'arena-demo:%'").bind(s(b.offerId),await hash(t)).first<any>();if(!v)fail('Questa non è la tua prova.',403)}else{v=(await ownedVenue()).v}if(!v)fail('Locale assente.');const o=await db().prepare("SELECT * FROM offers WHERE id=? AND venue_id=? AND status='collected'").bind(s(b.offerId),v.id).first<any>();if(!o||!o.proof_hash)fail('Registra prima il ritiro.');
  if(!env.FUJI_PRIVATE_KEY)fail('Registrazione Fuji non configurata. Il ritiro è già salvato nell’app.',503);
  const key='fuji:'+Math.floor(n/86400000);if(!o.tx_hash){const rate=await db().prepare('INSERT INTO limits(id,count,expires) VALUES(?,1,?) ON CONFLICT(id) DO UPDATE SET count=count+1 RETURNING count').bind(key,n+86400000).first<any>();if(rate.count>50)fail('Limite di 50 registrazioni di prova giornaliere raggiunto. Il ritiro resta salvato.',429);}
  try{const proof=await publishReceipt(env.FUJI_PRIVATE_KEY,'0x'+await hash(o.request_id),'0x'+o.proof_hash,o.tx_hash||undefined);await db().prepare('UPDATE offers SET tx_hash=? WHERE id=?').bind(proof.txHash,o.id).run();return respond(proof)}catch{fail('Fuji non è disponibile ora. Il ritiro resta salvato; riprova la registrazione.',503)}
 }
